...
class SQLighter:
    ...
    def update_last(self, id_user, last_menu, id_last_msg):
        """Обновление последнего меню и id последнего сообщения"""
        with self.connection:
            self.cursor.execute(f"UPDATE `UserAcess` SET `last_menu` = '{last_menu}' WHERE `id_user` = '{id_user}'")
            self.cursor.execute(f"UPDATE `UserAcess` SET `id_last_msg` = '{id_last_msg}' WHERE `id_user` = '{id_user}'")

    def get_all_user(self):
        """"""
        with self.connection:
            return self.cursor.execute("SELECT * FROM `UserAcess`").fetchall()
